...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
   ...
   if (freelist == NULL || freelist->head == NULL) {
        printf("Error: Free List is empty or uninitialized\n");
        return;
    }
    ...
    // Find a block in the free list that can accommodate the requested memory
    while (current != NULL) {
        blk = current->blk;
    ...
    if (found) {
        ...
        if (blk->end + 1 <= blk->end) {
            block_t *fragment = malloc(sizeof(block_t));
            fragment->pid = 0;
            fragment->start = blk->end + 1;
            fragment->end = blk->end;

            // Insert the fragment into the free list based on policy
            if (policy == 1) {
                list_add_to_back(freelist, fragment); // FIFO
            } else if (policy == 2) {
                list_add_ascending_by_address(freelist, fragment); // BESTFIT
            } else if (policy == 3) {
                list_add_descending_by_blocksize(freelist, fragment); // WORSTFIT
            }
        }
    } else {
    ...
    */
    ...
    if (found) {
        ...
        if (policy == 1) {
            ...
            list_add_ascending_by_blocksize(freelist, blk); // BESTFIT
        } else if (policy == 3) {
            list_add_descending_by_blocksize(freelist, blk); // WORSTFIT
        }
