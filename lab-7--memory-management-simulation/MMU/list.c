...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
   ...
   */
    ...
    if (c == NULL || newblk->start < c->blk->start) {
        newNode->next = c;
        l->head = newNode;
        return;
    }

    while (c->next != NULL && newblk->start > c->next->blk->start) {
        c = c->next;
    }
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   */
   node_t *newNode = node_alloc(newblk);
    ...
    if (c == NULL || compareSize(newblk->end - newblk->start, c->blk)) {
        newNode->next = c;
        l->head = newNode;
        return;
    }

    while (c != NULL && !compareSize(newblk->end - newblk->start, c->blk)) {
        c = c->next;
    }
...
void list_coalese_nodes(list_t *l){ 
   ...
   */
    ...
    while (current != NULL) {
        block_t *prevBlock = prev->blk;
        block_t *currentBlock = current->blk;

        if (compareSize(prevBlock->end + 1, currentBlock)) {
   ...
   * Look at list_is_in_by_size()*/
    ...
    while (current != NULL) {
        if (comparePid(pid, current->blk)) {
            return true; // Found a block with the given PID
        }
        current = current->next;
    }
