...
{ 
    ...
    // Initialize rem_time array
    for (int i = 0; i < n; i++)
        rem_time[i] = plist[i].bt; 
    
    while (completed < n) {
        int min_bt = INT_MAX; 
        
        for (int i = 0; i < n; i++) {
            if (rem_time[i] > 0 && rem_time[i] < min_bt && plist[i].art <= t) {
                min_bt = rem_time[i];
                min_index = i;
            }
        }
        ...
        if (rem_time[min_index] == 0) {
            completed++;
            plist[min_index].wt = t + 1 - plist[min_index].bt - plist[min_index].art;
            
            if (plist[min_index].wt < 0)
                plist[min_index].wt = 0;
        }
...
{ 
     ...
     */ 
      ProcessType *process_a = (ProcessType *)this;
    ProcessType *process_b = (ProcessType *)that;
